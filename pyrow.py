...
class pyrow(object):
    ...
    def get_monitor(self, forceplot=False):
        ...
        if forceplot:
            command.extend(['CSAFE_PM_GET_FORCEPLOTDATA',
                           32, 'CSAFE_PM_GET_STROKESTATE'])
        ...
        if monitor['power']:
            monitor['pace'] = (
                (2.8 / results['CSAFE_GETPOWER_CMD'][0]) ** (1./3)) * 500
            monitor['calhr'] = results['CSAFE_GETPOWER_CMD'][0] * \
                (4.0 * 0.8604) + 300.
        else:
        ...
        if forceplot:
            ...
            monitor['forceplot'] = results['CSAFE_PM_GET_FORCEPLOTDATA'][1:(
                datapoints+1)]
    ...
    def get_force_plot(self):
        ...
        command = ['CSAFE_PM_GET_FORCEPLOTDATA',
                   32, 'CSAFE_PM_GET_STROKESTATE']
        results = self.send(command)

        forceplot = {}
        datapoints = results['CSAFE_PM_GET_FORCEPLOTDATA'][0] / 2
        forceplot['forceplot'] = results['CSAFE_PM_GET_FORCEPLOTDATA'][1:(
            datapoints+1)]
    ...
    def get_erg(self):
        ...
        command = ['CSAFE_GETVERSION_CMD',
                   'CSAFE_GETSERIAL_CMD', 'CSAFE_GETCAPS_CMD', 0x00]
        results = self.send(command)
    ...
    def set_clock(self):
        ...
        command.extend(
            ['CSAFE_SETDATE_CMD', (now.year-1900), now.month, now.day])
    ...
    def set_workout(self, program=None, workout_time=None, distance=None, split=None,
        ...
        if program != None:
            ...
            command.extend(['CSAFE_SETHORIZONTAL_CMD',
                           distance, 36])  # 36 = meters
        ...
        if split != None:
            if workout_time != None and program == None:
                ...
                time_raw = workout_time[0]*3600 + \
                    workout_time[1]*60+workout_time[2]
                # split workout_time that will occur 30 workout_times (.01 sec)
                minsplit = int(time_raw/30*100+0.5)
                self.__checkvalue(split, "Split Time", max(
                    2000, minsplit), time_raw*100)
                ...
                minsplit = int(distance/30+0.5)
                self.__checkvalue(split, "Split distance",
                                  max(100, minsplit), distance)
